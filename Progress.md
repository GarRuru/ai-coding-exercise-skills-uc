...
### 11. Experiment-G：三 Spec 連跑（重跑 F，驗證已固化的 skill 修正）

**目標**：在全新 worktree（`.worktrees/experiment-G`，branch: `experiment-G`）中再次依序執行三個 spec，驗證先前固化的 skill 修正是否生效，並蒐集殘餘缺失。限制：必須用 `/execute-uc` skill、禁讀 git log/diff、所有檔案操作限定在 worktree 絕對路徑、僅跑 InMemory profile。
...
| 項目 | 結果 |
|------|------|
| 產生 `.java` 檔案 | 48（main 42 + test 6；未產生 TagGroup/Tag，YAGNI——create-pbi UC 用不到） |
| Pending 機制（ProductBacklogItem.json 套用後刪除、Team.json 保留） | ✅ |
| InMemory 測試 | ✅ ReadOnlyTaskTest 7/7、ReadOnlyProjectTest 5/5、CreateProductBacklogItemServiceTest 2/2 = **14/14** |
| Gate 2.5（Deterministic Review，對 pbi aggregate） | ✅ PASS 0 CRITICAL / 1 WARN（R-09 誤報） |

#### 11.2 過程中發現的 Skill 缺失（3 項，均屬「範例/workflow 模板 vs authority 規則」衝突）

| # | 缺失 | 根本原因 | 狀態 |
|---|------|---------|------|
| G-1 | Aggregate 首次編譯失敗：`not abstract and does not override abstract method getId()` | `examples/Plan.java` 沒宣告 `private PlanId id;` 也沒 `getId()`，與 `aggregate.md § Step 9.5`（authority，本來就正確）矛盾；`uc-workflow.md` Step 4.1 CRITICAL checks 也漏列此項 | ✅ 已修並 commit `8cce3f1` |
| G-2 | Gate 2.5 報 CRITICAL **F-02**：PATTERN 測試含 `@ActiveProfiles` | `uc-workflow.md` **Step P.4** 的 PATTERN 測試模板明文要求 `@ActiveProfiles("test-inmemory")`，與 **F-02 / ADR-021** 禁用規則衝突 | ⏳ worktree 測試已移除該註解（14/14 仍過）；**skill 模板 Step P.4 尚待修正** |
| G-3 | Gate 2.5 報 WARN R-09（DateProvider.now() 缺失於 Service） | 誤報——時間戳由 Aggregate 用 `DateProvider.now()` 產生，非 Service | 可忽略（WARN 不阻擋） |

#### 11.3 關鍵釐清：先前 experiment-D §7.3 的 Bug #5 其實是「範例誤導」

experiment-D 當時把「`private PbiId id` 欄位被誤刪」歸因為「誤以為 EsAggregateRoot 有 protected id」。經 experiment-G + `javap` 查證 `ezddd-entity 4.1.0`：`AggregateRoot` 確實**沒有** id 欄位、`Entity.getId()` 是 abstract。真正問題在 `Plan.java` 範例缺這兩樣（authority `aggregate.md` 本來就寫對）。→ 已由 G-1 commit 根治。

---

### 12. Skill Bug 修正（基於 Experiment-G 發現）

#### 12.1 G-1：Aggregate `id` 欄位 + `getId()`（已 commit）

**Commit**：`8cce3f1` — 2 files changed, 8 insertions(+)

| 檔案 | 修改內容 |
|------|---------|
| `references/examples/Plan.java` | 新增 `private PlanId id;`（附註說明 EsAggregateRoot 不提供）+ `@Override public PlanId getId()` |
| `references/uc-executor/uc-workflow.md` Step 4.1 | CRITICAL checks 新增「`id` field + `getId()` REQUIRED」，指向 authority `aggregate.md § Step 9.5` |

> authority `patterns/domain/aggregate.md`（§ Step 9.5）本身正確，未改動——只修正了與它矛盾的 consumer（範例 + workflow checklist）。

#### 12.2 G-2：uc-workflow Step P.4 的 `@ActiveProfiles` 模板（**待修**）

PATTERN 測試模板應移除 `@ActiveProfiles("test-inmemory")`，改由 `SPRING_PROFILES_ACTIVE` 環境變數 / TestSuite 控制 profile（與 F-02 / ADR-021 一致）。worktree 端已驗證移除後 14/14 仍通過。

#### 12.3 觀察：Gate 2.5 Stop-hook 與 git worktree 的落差

`gate25-stop-guard.sh` 以 `$PROJECT_DIR/src` 計算 checksum，但本實驗程式碼隔離在 worktree，主 repo 無 `src/`。真正的 Gate 2.5 已在 worktree 對實際程式碼跑過（marker checksum `24663f3b…`，0 CRITICAL）；主 repo 端補了對應 handshake marker（`files_checksum: "empty"`）以放行 session。未來若常用 worktree 跑實驗，可考慮讓 stop-guard 支援 worktree 路徑。

---

## 待執行項目

- [ ] **修正 uc-workflow.md Step P.4**：移除 PATTERN 測試模板的 `@ActiveProfiles`（G-2）
